...
simData <- foreach(i=1:10000, .combine=rbind) %do% {
  dt<-datagen_cont(n=150, m=15, K=2, cv=0, sigmac=matrix(c(1,0.01,0.01,1),2), sigmae= matrix(c(1,0.01,0.01,1),2), eff=c(1,0.5), time.eff=c(1.1,1.3,1.4,0.9,0.7,0.5))$short

  lme1<-lme(out1~factor(time)+arm,random=~1|cluster,data=dt,control=lmeControl(returnObject=TRUE))
  lme2<-lme(out2~factor(time)+arm,random=~1|cluster,data=dt,control=lmeControl(returnObject=TRUE))
  formula1=formula(lme1)
  formula2=formula(lme2)

  fitEM<-EM.estim(dt,formula1,formula2)
...
write.table(simData, file="/Users/kdavis07/Desktop/test.EM.results10k.txt", sep="\t", row.names=F)