...
###############################################
datagen_cont <- function(n, m, K, cv, rho01, rho2, vars, eff, time.eff ){
  
  library(MASS)
  
  # sigmae: covariance matrix for errors
  rho0k <- diag(rho01)
  ...
  Y <- sweep((time.effect + matrix(temp2$arm) %*% matrix(eff,nrow=1) +gamma),2,c(sqrt(sigmae[1,1]),sqrt(sigmae[2,2])),FUN="*")+epsilon
